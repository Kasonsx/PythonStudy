...
class BookmarkSpider(scrapy.Spider):
	...
	def parse(self, response):
		for work in response.css("li.image-item + a::attr(href)"):
			yield response.follow(work, self.parse_work)

	def parse_work(self, response):
		yield{
			'url': response.css("div.works_display > img::attr(src)")
		} 